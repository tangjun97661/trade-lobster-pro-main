...
async def run_single_browser_task(
    ...
    try:
        ...
        context_config = BrowserContextConfig(
            save_downloads_path=os.getenv("OUTPUT_PATH", "./tmp/downloads"),
            window_height=window_h,
            window_width=window_w,
        ...
        try:
            desired = None
            m = re.search(r'filename\s*[:=]\s*["\']?([^"\'\n\r]+?)["\']?(?:\s|$)', task_query, flags=re.IGNORECASE)
            if m:
                desired = os.path.basename(m.group(1).strip())
            if desired:
                dl_dir = os.getenv("OUTPUT_PATH", "./tmp/downloads")
                if os.path.isdir(dl_dir):
                    entries = []
                    for f in os.listdir(dl_dir):
                        p = os.path.join(dl_dir, f)
                        lower = f.lower()
                        if os.path.isfile(p) and not any(lower.endswith(ext) for ext in ['.crdownload', '.part', '.partial', '.tmp']):
                            try:
                    ...
                    if entries:
                        ...
                        latest = entries[0][0]
                        target = os.path.join(dl_dir, desired)
                        if os.path.abspath(latest) != os.path.abspath(target):
                            if os.path.exists(target):
                                os.remove(target)
                            os.replace(latest, target)
                            logger.info(f"Renamed latest download to {target}")
        except Exception as e:
            logger.error(f"Rename after deep research download failed: {e}", exc_info=True)
