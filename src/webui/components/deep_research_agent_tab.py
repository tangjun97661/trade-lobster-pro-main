...
async def run_deep_research(webui_manager: WebuiManager, components: Dict[Component, Any]) -> AsyncGenerator[
    ...
    yield {
        ...
        markdown_display_comp: gr.update(value="正在启动研究..."),
    ...
    try:
        ...
        if not llm:
            raise ValueError("模型初始化失败，请检查“智能体设置”。")
        ...
        if report_file_path and os.path.exists(report_file_path):
            ...
            final_ui_update[markdown_download_comp] = gr.update(value=None, label="下载研究报告",
                                                                interactive=False)
        else:
            logger.warning("Final report file not found and not in result dict.")
            final_ui_update[markdown_display_comp] = gr.update(value="# 研究完成\n\n*未找到最终报告。*")
        ...
        gr.Error(f"研究失败：{e}")
        yield {markdown_display_comp: gr.update(value=f"# 研究失败\n\n**错误：**\n```\n{e}\n```")}
        ...
        yield {
            start_button_comp: gr.update(value="▶️ 开始", interactive=True),
...
async def stop_deep_research(webui_manager: WebuiManager) -> Dict[Component, Any]:
    ...
    final_update = {
        stop_button_comp: gr.update(interactive=False, value="⏹️ 正在停止...")
    }
...
def create_deep_research_agent_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        research_task = gr.Textbox(label="研究任务", lines=5,
                                   value="请给我一份 6 月 1 日到 10 日瑞士旅行的详细行程规划。",
                                   interactive=True)
        with gr.Row():
            resume_task_id = gr.Textbox(label="续跑任务 ID", value="",
                                        interactive=True)
            parallel_num = gr.Number(label="并行智能体数量", value=1,
                                     precision=0,
                                     interactive=True)
            max_query = gr.Textbox(label="研究保存目录", value="./tmp/deep_research",
                                   interactive=True)
    with gr.Row():
        stop_button = gr.Button("⏹️ 停止", variant="stop", scale=2)
        start_button = gr.Button("▶️ 开始", variant="primary", scale=3)
    with gr.Group():
        markdown_display = gr.Markdown(label="研究报告")
        markdown_download = gr.File(label="下载研究报告", interactive=False)
    ...
    async def start_wrapper(*values) -> AsyncGenerator[Dict[Component, Any], None]:
        comps = {comp: val for comp, val in zip(all_managed_inputs, values)}
